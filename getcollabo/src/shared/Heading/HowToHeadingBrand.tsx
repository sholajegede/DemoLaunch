...
export interface HowToHeadingBrandProps extends HTMLAttributes<HTMLHeadingElement> {
  fontClass?: string;
  desc?: ReactNode;
  hasNextPrev?: boolean;
  isCenter?: boolean;
...
const HowToHeadingBrand: React.FC<HowToHeadingBrandProps> = ({
  children,
  desc = "",
  ...
  return (
    <div
      className={`nc-Section-HowToHeadingBrand relative flex flex-col sm:flex-row sm:items-end justify-between ${className}`}
    >
      <div
        className={
      ...
      >
        <h2 className={`text-2xl md:text-2xl font-semibold`} {...args}>
          {children || `Section HowToHeadingBrand`}
        </h2>
        {desc && (
...
export default HowToHeadingBrand;
