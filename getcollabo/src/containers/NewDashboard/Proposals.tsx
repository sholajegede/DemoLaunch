...
import PageProposalsInfluencer from "containers/Proposals/PageProposalsInfluencer";

export interface ProposalsProps {
  className?: string;
}

const Proposals: FC<ProposalsProps> = ({ className = "" }) => {
  ...
  return (
    <>
      {influencerProfile.verified === false ? (
        ...
        <div
          className={`nc-Proposals overflow-hidden relative ${className}`}
          data-nc-id="Proposals"
        >
          <Helmet>
          ...
          <div className="p-4 pb-32 sm:ml-64">
            <PageProposalsInfluencer />
          </div>
        </div>
      )}
    </>
